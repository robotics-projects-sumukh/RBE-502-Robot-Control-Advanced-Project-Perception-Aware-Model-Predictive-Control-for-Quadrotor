...
def calc_quat_cost(qk, qd, weight):
  q_aux = cs.vertcat(
  ...
  q_att_denom = cs.sqrt(q_aux[0] * q_aux[0] + q_aux[3] * q_aux[3] + 1e-3)
  q_att = cs.vertcat(
    q_aux[0] * q_aux[1] - q_aux[2] * q_aux[3],
    q_aux[0] * q_aux[2] + q_aux[1] * q_aux[3],
    q_aux[3]) / q_att_denom
  return cs.transpose(q_att) @ weight @ q_att

def calc_square_cost(udes_, ucurr_, sqr_weight):
  cost_square = 0
